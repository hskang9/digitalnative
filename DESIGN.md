...
Arrow keys navigate sections through the same room-selection path as links: Left/Right select the previous/next room; Up/Down do the same outside reading, but retain native scrolling while reading. Navigation stops at Garden and Contact without wrapping, ignores held-key repeats, and respects inputs, media controls, text selection, and modified browser shortcuts. Separate rapid presses use the requested destination during smooth tour travel; wheel, touch, or pointer input clears that pending destination. A desktop hint exposes Left/Right navigation. Drag-to-look remains bounded; Home resets a dragged view only when the scene itself has focus and the tour is off.
