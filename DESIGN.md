...
Room handoffs use a 350ms outgoing fade, camera travel, then a 400ms reveal with a restrained 12px rise and 2px cross-blur. The reveal animates the inner HTML surface so it does not overwrite CSS3D's camera transform. Outgoing walls remain rendered but inert until the fade completes. The scroll camera continuously follows its rendered position across segment boundaries, including fast skips and reverse scrolling; incoming content waits until the camera has nearly settled. Reading remains stationary. Both the OS preference and Motion off remove these panel transitions.
